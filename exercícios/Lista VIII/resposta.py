...
xldsnqds xrsrjv xrqhp xxzdlwzx xxtlt xxcfpp xqbs xvqnd xvgkv xdlsm xgj xggzgkln xftgxfpj 
xffjdlfs kzzsbq kztrcqxt kmbgdwm kmk kbg ktbcx kszjq ksptr kswfwbzv kslnx khzzw
...
def prepositions(txt):
	p = 0 #prepositions_count
	for word in txt:
		if word[0] in zumbi and word[-1] not in zumbi:
			p += 1
	return p

print("01. A;%d" %prepositions(txtB))

#question 2 + 3


def is_verb(word):
	if len(word) == 7 and word[-1] not in zumbi:
		return True
	else:
		return False
def first_person(verb):
	if verb[1] not in zumbi:
		return True
	else:
		return False

v, vp = 0, 0 #v: verbs, vp: verbs in first person
for word in txtB:
	if is_verb(word):
		v += 1
		if first_person(word):
			vp += 1
...
#dicionario relacionado o grau de organização para cada caracter em Googlon
dic = dict()
k = 0
while (k < len(order)):
	c = order[k]
...
def more_sort(array):
	...
	for word in array:
		if word != -1:
			#atribuindo a string menor relativa do laço
			if len(word_sort) < len(word):
				smaller = len(word_sort)
			else:
				smaller = len(word)
			#laço comparativo c/ 'grau' de organização entre uma palavra do laço qualquer e a última palavra mais 'organizada'
...
listaB = list()
while (True):
	word_sort = more_sort(txtB)
	listaB.append(word_sort)


	x = txtB.index(word_sort)
	txtB[x] = -1

	if len(listaB) == len(txtB):
		break
print("04. D;", listaB, '\n')		

#question 5

def convert_decimal(g_word):
	number = 0
	k = 0
	while (k < len(g_word)):
		x = dic[g_word[k]]
...
def is_magic(g_word):
	...
	for c in g_word:
		if c not in array:
			...
			break
	if len(g_word) == len(array):
		if convert_decimal(g_word) % 42 == 0:
			return True
	return False
...
print("5. E;%d -> " %(len(magic_numbers)), end='')
